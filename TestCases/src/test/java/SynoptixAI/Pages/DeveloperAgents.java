...
public class DeveloperAgents {
	...
	private By agentCategory=By.xpath("//span[normalize-space()='Marketing' or normalize-space()='All Teams']");
	private By developerCategory=By.xpath("//button[contains(text(),'Developer')]");
	private By codeDebugAgent=By.xpath("(//button[contains(text(),'Try Now')])[1]");
	private By codeGeneratorAgent=By.xpath("(//button[contains(text(),'Try Now')])[2]");
	...
	public void checkCodeDebug() throws InterruptedException 
	{
		...
		wait.until(ExpectedConditions.elementToBeClickable(developerCategory)).click();

		Thread.sleep(2000);

		// Click Code Debug Agent
		wait.until(ExpectedConditions.elementToBeClickable(codeDebugAgent)).click();
	...
	public void checkCodeGenerator() throws InterruptedException 
	{
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", agentLibBtn);
		Thread.sleep(1500);
		wait.until(ExpectedConditions.elementToBeClickable(codeGeneratorAgent)).click();
